...
import html  # <-- Added import for HTML escaping
...
def setup_nlp_resources():
    """Downloads necessary NLTK and SpaCy resources if not already present."""
    try:
    ...
    # --- MODIFIED: Use a larger model with word vectors for semantic similarity ---
...
class AdvancedATSCalculator:
    def __init__(self):
        ...
        # --- NEW: Lists for analyzing bullet point quality ---
        self.action_verbs = [
    ...
    # --- UNCHANGED: Text extraction methods are robust ---
    ...
    # --- FIXED: Correctly identifies and stores specific phrases for highlighting ---
    def analyze_work_experience(self, text):
        ...
        # Regex to find numbers, percentages, dollar amounts, etc.
        ...
        for line in lines:
            # Find Quantified Metrics by iterating over all matches in a line
            for match in re.finditer(quant_pattern, line):
                found_metrics.add(match.group(0))

            # Check for action verbs at the start of a line (case-insensitive)
            stripped_line = line.strip()
            for verb in self.action_verbs:
                # Match verb at the start of the line, ignoring bullets/spaces
                if re.match(r'^[•\*\-\s]*' + verb + r'\b', stripped_line, re.IGNORECASE):
                    # Extract the actual verb from the original line to preserve its casing
                    verb_match = re.search(r'\b' + verb + r'\b', stripped_line, re.IGNORECASE)
                    if verb_match:
                        found_verbs.add(verb_match.group(0))
                    break # A line can only start with one verb for this analysis

            # Check for passive phrases anywhere in the line (case-insensitive)
            for phrase in self.passive_phrases:
                if phrase.lower() in line.lower():
                    # Find the actual phrase in the original line to preserve its casing
        ...
        # Scoring is now based on counts of specific items, which is more accurate.
    ...
    # --- UNCHANGED (remainder of helper methods) ---
    ...
    def generate_ai_suggestions(self, results, job_description):
        if not os.getenv("GOOGLE_API_KEY") or not job_description or not results.get('experience_analysis'): return ""
    ...
    # --- FIXED: Rewritten to properly apply HTML highlights to specific phrases ---
    def generate_highlighted_resume_html(self, results):
        ...
        # Create a combined list of all phrases to highlight, with their corresponding HTML class and title.
        ...
        # Sort phrases by length in descending order. This is crucial to avoid issues where
        # a short phrase is a substring of a longer one (e.g., replacing "for" before "responsible for").
        phrases_to_highlight.sort(key=lambda x: len(x[0]), reverse=True)

        # First, escape the entire raw text to prevent any user-inputted HTML from breaking the layout.
        safe_text = html.escape(text)

        # Sequentially apply the replacements.
        for phrase, class_name, title_text in phrases_to_highlight:
            ...
            # Use case-insensitive regex with word boundaries (\b) to replace whole words only.
            # This prevents replacing 'achieved' inside 'overachieved'.
            try:
                safe_text = re.sub(r'\b' + re.escape(safe_phrase) + r'\b', replacement_html, safe_text, flags=re.IGNORECASE)
            except re.error:
                # Fallback for complex phrases with symbols that might break a simple regex rule.
